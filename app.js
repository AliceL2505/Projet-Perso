...
function getProfiles() {
  try {
    ...
    if (raw) {
      ...
      // Ne garde que les profils toujours proposés par l'application : permet de retirer
      // proprement un profil (ex. suppression du profil "Lucie") côté navigateur aussi.
      parsed = parsed.filter(p => DEFAULT_PROFILES.some(dp => dp.id === p.id));
...
function getActiveProfileId() {
  const id = localStorage.getItem(ACTIVE_PROFILE_KEY) || "alice";
  // Le profil "Lucie" a été supprimé ; on retombe sur Alice si c'était le profil actif.
  return id === "lucie" ? "alice" : id;
}
...
// Suppression définitive des données de l'ancien profil "Lucie" (retiré à la demande
// de la personne concernée) : on efface ce qui a pu être stocké localement.
localStorage.removeItem("cahierBudget_v1__lucie");
...
function loadState(profileId) {
  ...
  // Profil inconnu : jeu de données vide par défaut (garde-fou).
  return {
    categories: cloneDefaultCategories(),
    transactions: [],
    projects: [],
